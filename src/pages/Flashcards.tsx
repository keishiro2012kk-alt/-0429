  ...
  if (words.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-slate-500">
        <p>No data. Please upload a PDF from the word list.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 items-center">
      <header className="w-full text-left">
        ...
        <p className="text-slate-400 mt-1">Tap the card to flip it.</p>
      </header>
      ...
      >
        ...
        >
          {/* Front — 日本語訳を表示（問題面） */}
          <div
            className="absolute inset-0 glass-panel flex flex-col items-center justify-center p-8 text-center border-t border-white/20 shadow-2xl group-hover:border-cyan-500/50 transition-all"
            style={{ backfaceVisibility: "hidden" }}
          >
            <div className="absolute top-4 right-4 z-10 flex gap-2">
              <button
                onClick={(e) => { e.stopPropagation(); updateWord(word.id!, { isFavorite: !word.isFavorite }); }}
            ...
            {word.exampleTranslation && (
              <p className="text-sm text-slate-400 mt-4 italic">{word.exampleTranslation}</p>
            )}
            <div className="mt-10 text-slate-500 flex items-center gap-2">
              <RotateCw className="w-4 h-4" /> <span className="text-xs font-bold uppercase tracking-widest">Tap to flip</span>
            </div>
          </div>
      ...
      <div className="flex gap-4 mt-6 w-full max-w-sm">
        <button onClick={goPrev} className="flex-1 py-4 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl transition-colors text-slate-300 flex justify-center items-center">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <button onClick={goNext} className="flex-1 py-4 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl transition-colors text-slate-300 flex justify-center items-center">
