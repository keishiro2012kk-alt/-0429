...
export default function ActiveQuiz() {
  ...
  useEffect(() => {
    async function loadQuiz() {
      ...
      try {
        if (state?.mode === "grammar") {
          const requestData = { mode: "grammar", data: state.grammarTopic, count: 10 };
          ...
          if (words.length === 0) {
            setError(state?.mode === "favorites" ? "お気に入りに登録された単語がありません。" : state?.mode === "weakness" ? "間違えたことのある単語がありません。素晴らしい！" : "単語データがありません。先にPDFをアップロードしてください。");
            setLoading(false);
            return;
          }
          const generatedQuestions = words.map(w => {
            ...
            return {
              id: w.id as string,
              type: "fill_in_blank",
              questionText: w.exampleTranslation,
              subText: blanked,
    ...
    loadQuiz();
  }, [state, allWords, wordsLoading, user]);
  ...
  const nextQuestion = async () => {
    if (currentIndex < questions.length - 1) {
      ...
      await addSession({
        ...
        topic: state?.mode === "grammar" ? state.grammarTopic : "words",
        correctAnswers: correctCount,
        totalQuestions: questions.length,
        durationMinutes: 5 // mock for now
      });
      navigate("/stats");
    }
  };
  ...
  const startListening = () => {
    ...
    recognition.onresult = (e: any) => {
       ...
       setInputValue(transcript.toLowerCase().replace(/[^a-z0-9 ]/g, ''));
    };
  ...
  if (loading || wordsLoading) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
      ...
      <p className="font-bold text-cyan-400 tracking-widest animate-pulse">問題を生成中...</p>
    </div>
  );
  ...
  if (error) return (
    <div className="glass-panel p-6 border-red-500/30 text-center">
      ...
      <button onClick={() => navigate("/quiz")} className="btn-neon mt-6">中断する</button>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="glass-panel p-8 relative overflow-hidden flex flex-col items-center text-center">
        {feedback === "correct" && <div className="absolute inset-0 bg-green-500/10 pointer-events-none flex flex-col items-center justify-center animate-in fade-in duration-300"><div className="w-full absolute bottom-0 h-1 bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.8)]"></div></div>}
        {feedback === "incorrect" && <div className="absolute inset-0 bg-red-500/10 pointer-events-none flex flex-col items-center justify-center animate-in fade-in duration-300"><div className="w-full absolute bottom-0 h-1 bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.8)]"></div></div>}
        
        <span className="text-xs font-bold tracking-widest text-slate-500 mb-6 border border-white/5 px-3 py-1 rounded-full bg-black/40 shadow-inner">
          {typeTranslation[q.type] || q.type.replace(/_/g, " ")}
        </span>
        ...
        {q.hint && <p className="text-sm text-purple-400 mt-2 mb-8 italic">ヒント: {q.hint}</p>}

        <form onSubmit={handleSubmit} className={cn("w-full max-w-sm relative", !q.hint && !q.subText && "mt-8")}>
          <input 
            ref={inputRef}
            type="text"
            className={cn("input-glass text-center text-xl font-mono tracking-wider", 
              feedback === "correct" && "border-green-400/50 focus:border-green-400 text-green-400",
              feedback === "incorrect" && "border-red-400/50 focus:border-red-400 text-red-400"
            )}
            placeholder="答えを入力..."
            value={inputValue}
          ...
          {feedback === "pending" && (
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
               ...
               <button type="submit" className="p-2 hover:bg-white/10 rounded-lg text-cyan-400 hover:text-cyan-300 transition">
